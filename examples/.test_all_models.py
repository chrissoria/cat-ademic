...
Usage:
    python examples/.test_all_models.py [--free-only] [--paid-only] [--model MODEL_NAME]

API keys are loaded from:
    /Users/chrissoria/Documents/Research/Categorization_AI_experiments/.env

Output saved to:
    examples/test_output/model_test_results_<timestamp>.csv
"""
...
ENV_FILE = Path("/Users/chrissoria/Documents/Research/Categorization_AI_experiments/.env")
...
def main():
    ...
    # Save results
    if not args.no_save:
        output_path = OUTPUT_DIR / f"model_test_results_{timestamp}"
